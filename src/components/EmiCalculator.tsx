...
export default function EmiCalculator({ lang, t, settings, defaultPrice }: EmiCalculatorProps) {
  const activeBasePrice = defaultPrice || settings?.calcDefaultPrice || 32000;
  const activeDownpaymentPct = settings?.calcDefaultDownPaymentPct || 30;
  const activeInterestRate = settings?.calcBaseInterestRate || 0;

  const [price, setPrice] = useState<number>(activeBasePrice);
  const [downPayment, setDownPayment] = useState<number>(() => Math.floor(activeBasePrice * (activeDownpaymentPct / 100)));
  const [months, setMonths] = useState<number>(6);
  const [interestRate, setInterestRate] = useState<number>(activeInterestRate); // Default 0% interest for promotions
  const [monthlyEmi, setMonthlyEmi] = useState<number>(0);
  const [remaining, setRemaining] = useState<number>(0);

  // Sync state with settings and defaultPrice updates
  useEffect(() => {
    const freshPrice = defaultPrice || settings?.calcDefaultPrice || 32000;
    const freshPct = settings?.calcDefaultDownPaymentPct || 30;
    const freshInterest = settings?.calcBaseInterestRate || 0;

    setPrice(freshPrice);
    setDownPayment(Math.floor(freshPrice * (freshPct / 100)));
    setInterestRate(freshInterest);
  }, [defaultPrice, settings]);
    ...
    <div id="emi-calculator-panel" className="bg-slate-50 rounded-2xl p-6 border border-slate-200 shadow-inner">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 p-5 bg-white rounded-xl border border-slate-100 shadow-sm mb-4">
        <div className="flex flex-col">
          ...
          <span className="text-2xl font-mono font-bold text-slate-800">₹ {(remaining || 0).toLocaleString()}</span>
        ...
        <div className="flex flex-col border-t md:border-t-0 md:border-x border-slate-100 pt-3 md:pt-0 md:px-5">
          ...
          <span className="text-2xl font-mono font-bold text-indigo-600">₹ {(monthlyEmi || 0).toLocaleString()} / {lang === "bn" ? "মাস" : "mo"}</span>

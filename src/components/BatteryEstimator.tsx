...
export default function BatteryEstimator({ lang, settings }: BatteryEstimatorProps) {
  const isBng = lang === "bn";

  // State variables for the estimator with dynamic fallbacks from CMS settings
  const [voltage, setVoltage] = useState<number>(() => settings?.calcDefaultVoltage || 60);
  const [capacity, setCapacity] = useState<number>(() => settings?.calcDefaultCapacity || 40);
  const [speed, setSpeed] = useState<number>(() => settings?.calcDefaultSpeed || 35); // km/h
  ...
  // Sync state with settings dynamically when loaded or changed in ERP
  useEffect(() => {
    if (settings) {
      if (settings.calcDefaultVoltage) setVoltage(settings.calcDefaultVoltage);
      if (settings.calcDefaultCapacity) setCapacity(settings.calcDefaultCapacity);
      if (settings.calcDefaultSpeed) setSpeed(settings.calcDefaultSpeed);
    }
  }, [settings]);

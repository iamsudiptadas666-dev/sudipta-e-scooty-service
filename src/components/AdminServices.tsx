...
import { Booking, Product, Customer, StaffMember } from "../types";
...
export default function AdminServices({ bookings, products, customers, staffMembers, onAddBooking, onUpdateBooking, onDeleteBooking, onLaunchInvoice, lang, t }: AdminServicesProps) {
  const isBng = lang === "bn";
  ...
  const [technicianName, setTechnicianName] = useState(staffMembers[0]?.name || "Unassigned");
  ...
  const resetForm = () => {
    ...
    setTechnicianName(staffMembers[0]?.name || "Unassigned");
    setRepairDetails("");
    setServiceCharge(0);
  };
  ...
  const handleStartEdit = (b: Booking) => {
    ...
    setEditingServiceCharge(b.serviceCharge || 0);
                  ...
                  <div className="flex justify-between items-center pt-2 border-t border-slate-100">
                    <div>
                      ...
                      <strong className="text-lg font-mono font-bold text-slate-800">
                        ₹{" "}
                        {isEditing
                          ? (editingParts.reduce((s, i) => s + ((i.price || 0) * (i.quantity || 0)), 0) + (editingServiceCharge || 0)).toLocaleString()
                          : b.totalAmount?.toLocaleString() || (b.serviceCharge || 0).toLocaleString()}
                      </strong>
                    </div>
                      ...
                      {isEditing ? (
                        ...
                        <>
                          ...
                          {onDeleteBooking && (
                            <button
                              onClick={() => {
                                if (confirm(isBng ? "আপনি কি নিশ্চিত যে এই জব কার্ডটি রিসাইকেল বিনে পাঠাতে চান?" : "Are you sure you want to move this Job Card to Recycle Bin?")) {
                                  onDeleteBooking(b.id);
                                }
                              }}
                              className="p-1.5 bg-rose-50 hover:bg-rose-100 text-rose-600 rounded-xl transition border border-rose-200 cursor-pointer"
                              title={isBng ? "জব কার্ড মুছে ফেলুন (ট্র্যাশ)" : "Delete Job Card (Trash)"}
                            >
                              <Trash className="w-3.5 h-3.5" />
                            </button>
                          )}

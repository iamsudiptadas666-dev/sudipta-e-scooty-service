...
export default function AdminProducts({ products: propProducts = [], onAdd, onUpdate, onDelete, lang, t }: AdminProductsProps) {
  ...
  const displayProducts = (propProducts && propProducts.length > 0) ? propProducts : localProducts;
  ...
  const handleEditClick = (p: Product) => {
    ...
    setImgUrl((p.images && p.images[0]) || p.image || "");
                ...
                return (
                  <tr key={p.id} className={`hover:bg-slate-50/50 transition ${isLowStock ? "bg-rose-50/20" : ""}`}>
                    <td className="p-4 w-16">
                      <img
                        src={(p.images && p.images[0]) || p.image || "https://images.unsplash.com/photo-1619642751034-765dfdf7c58e?w=200"}
                        alt={p.titleEng}
